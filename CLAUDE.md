...
- **`frontend/components/` 하위 컴포넌트를 Write/Edit한 직후, `form-validation-tests` 스킬을 반드시 호출한다. 훅 메시지(`[MANDATORY]`)가 출력되면 즉시 실행한다. 스킬 실행 없이 "완료"를 선언하지 않는다. 폼 여부는 스스로 판단하지 않는다 — 스킬이 판단한다.**
